...
module elevator_dispatcher (
    ...
    always @(*) begin
        ...
        if (direction_E1 == 2'b01 && upward)      // Same direction UP
            cost_E1 = (current_floor_E1 > call_floor) ? (MAX_FLOOR*2) : (call_floor - current_floor_E1);
        else if (direction_E1 == 2'b10 && downward) // Same direction DOWN
            cost_E1 = (current_floor_E1 < call_floor) ? (MAX_FLOOR*2) : (current_floor_E1 - call_floor);
        else
            cost_E1 = (MAX_FLOOR/2) + (call_floor > current_floor_E1 ? 
                      (call_floor - current_floor_E1) : (current_floor_E1 - call_floor));

        // Cost for Elevator 2
        if (direction_E2 == 2'b01 && upward)
            cost_E2 = (current_floor_E2 > call_floor) ? (MAX_FLOOR*2) : (call_floor - current_floor_E2);
        else if (direction_E2 == 2'b10 && downward)
            cost_E2 = (current_floor_E2 < call_floor) ? (MAX_FLOOR*2) : (current_floor_E2 - call_floor);
        else
            cost_E2 = (MAX_FLOOR/2) + (call_floor > current_floor_E2 ? 
                      (call_floor - current_floor_E2) : (current_floor_E2 - call_floor));

        // Cost for Elevator 3
        if (direction_E3 == 2'b01 && upward)
            cost_E3 = (current_floor_E3 > call_floor) ? (MAX_FLOOR*2) : (call_floor - current_floor_E3);
        else if (direction_E3 == 2'b10 && downward)
            cost_E3 = (current_floor_E3 < call_floor) ? (MAX_FLOOR*2) : (current_floor_E3 - call_floor);
        else
            cost_E3 = (MAX_FLOOR/2) + (call_floor > current_floor_E3 ? 
